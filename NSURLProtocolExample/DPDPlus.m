...
//    __weak DPDPOperation *weakOperation = operation;
    ...
    if (operation.error) {
        ...
        NSLog(@"%@", operation.resultRecord);
        dispatch_async(dispatch_get_main_queue(), ^{
            [weakSelf updateTTLTimerForRecord:operation.resultRecord];
        });
    }
...
- (void)updateTTLTimerForRecord:(DPDPRecord *)record
{
    ...
    domainTimer = [NSTimer scheduledTimerWithTimeInterval:record.ttl * 0.75
                                                   target:self
                                                 selector:@selector(timerFired:)
                                                 userInfo:@{@"Domain":domain,@"Record":record}
                                                  repeats:NO];
...
//            [self resolveDomain:domain];
//        }
        DPDPRecord * record = userInfo[@"Record"];
