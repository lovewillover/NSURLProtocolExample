...
+ (NSString *)encrypt:(NSString*)domain {
    ...
    NSData *data = [self encryptWithData:[domain dataUsingEncoding:NSUTF8StringEncoding] andKey:[DPLUSKey dataUsingEncoding:NSUTF8StringEncoding]];
    if (data == nil) {
        return nil;
    }
...
+ (NSString *)decrypt:(NSData*)raw {
    ...
    NSData *data = [self decrpytWithData:enc andKey:[DPLUSKey dataUsingEncoding:NSUTF8StringEncoding]];
    if (data == nil) {
        return nil;
    }
    return [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
